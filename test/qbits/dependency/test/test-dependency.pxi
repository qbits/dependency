...
(t/deftest t-transitive-dependencies
  (t/assert= #{:a :c :b}
             (transitive-dependencies g1 :d))
  (t/assert= '#{two four six one five three}
             (transitive-dependencies g2 'seven)))

(t/deftest t-transitive-dependencies-deep
  (t/assert=
   #{:level0
     :level1a :level1b :level1c :level1d
     :level2
...
(t/deftest t-transitive-dependents
  (t/assert= '#{four seven}
             (transitive-dependents g2 'five))
  (t/assert= '#{four seven six three}
...
(t/deftest t-before
  (t/assert (true? (before? [:a :b :c :d] :a :b)))
  (t/assert (true? (before? [:a :b :c :d] :b :c)))
  (t/assert (false? (before? [:a :b :c :d] :d :c)))
